...
const char *WINDOWTITLE = {"OpenCL/OpenGL Particle System -- Bailey Posante"};
const char *GLUITITLE = {"User Interface Window"};

// random parameters:

const float XMIN = {-800.0};
const float XMAX = {800.0};
const float YMIN = {0};
...
void Buttons(int id) {
  ...
  switch (id) {
    ...
    case RESET:
      Reset();
      ResetParticles();
      status = clEnqueueWriteBuffer(CmdQueue, dVel, CL_FALSE, 0,
                                    4 * sizeof(float) * NUM_PARTICLES, hVel, 0,
                                    NULL, NULL);
...
void DisplayScene() {
  ...
  if (scale2 < MINSCALE) scale2 = MINSCALE;
  glScalef((GLfloat)scale2, (GLfloat)scale2, (GLfloat)scale2);

  glDisable(GL_FOG);

  if (AxesOn != GLUIFALSE) glCallList(AxesList);
  ...
  // ****************************************
  // Here is where you draw your spheres.  You define them, however, in the
  // InitLists( ) function.
  // ****************************************
  ...
  if (ShowPerformance) {
    char str[128];
    sprintf(str, "%6.1f GigaParticles/Sec",
...
void InitCL() {
  ...
  if (status != CL_SUCCESS) fprintf(stderr, "clCreateContext failed: %d\n", status);

  // 4. create an opencl command queue:

  CmdQueue = clCreateCommandQueue(Context, Device, 0, &status);
  if (status != CL_SUCCESS) fprintf(stderr, "clCreateCommandQueue failed: %d\n", status);

  // create the velocity array and the opengl vertex array buffer and color
  // array buffer:
  ...
  glBindBuffer(GL_ARRAY_BUFFER, hPobj);
  glBufferData(GL_ARRAY_BUFFER, 4 * NUM_PARTICLES * sizeof(float), NULL,
               GL_STATIC_DRAW);
  ...
  dVel = clCreateBuffer(Context, CL_MEM_READ_WRITE,
                        4 * sizeof(float) * NUM_PARTICLES, NULL, &status);
  PrintCLError(status, "clCreateBuffer: ");

  // 6. enqueue the command to write the data from the host buffers to the
  // Device buffers:
  ...
  Program = clCreateProgramWithSource(Context, 1, (const char **)strings, NULL,
                                      &status);
...
void InitLists() {
  ...
  glColor3f(.5f, .3f, .7f);  // 0. <= r,g,b <= 1.
  glPushMatrix();
  glTranslatef(-1000.f, -800.f, 0.f);
...
void ResetParticles() {
  glBindBuffer(GL_ARRAY_BUFFER, hPobj);
  struct xyzw *points =
      (struct xyzw *)glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY);
  ...
  glBindBuffer(GL_ARRAY_BUFFER, hCobj);
  struct rgba *colors =
      (struct rgba *)glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY);
  for (int i = 0; i < NUM_PARTICLES; i++) {
...
bool IsCLExtensionSupported(const char *extension) {
  ...
  clGetDeviceInfo(Device, CL_DEVICE_EXTENSIONS, extensionSize, extensions,
                  NULL);
  ...
  for (char *start = extensions;;) {
    ...
    char *terminator =
        where + strlen(extension);  // points to what should be the separator

    if (*terminator == ' ' || *terminator == '\0' || *terminator == '\r' ||
        *terminator == '\n') {
      delete[] extensions;
      return true;
    }
...
void PrintOpenclInfo() {
  ...
  for (int p = 0; p < (int)numPlatforms; p++) {
    ...
    // find out how many devices are attached to each platform and get their
    // ids:
    ...
    if (status != CL_SUCCESS) fprintf(stderr, "clGetDeviceIDs failed (2)\n");

    fprintf(stderr, "\tNumber of Devices = %d\n", numDevices);

    cl_device_id *devices = new cl_device_id[numDevices];
    status = clGetDeviceIDs(platforms[p], CL_DEVICE_TYPE_ALL, numDevices,
                            devices, NULL);
    if (status != CL_SUCCESS) fprintf(stderr, "clGetDeviceIDs failed (2)\n");
    ...
    for (int d = 0; d < (int)numDevices; d++) {
      ...
      clGetDeviceInfo(devices[d], CL_DEVICE_MAX_COMPUTE_UNITS, sizeof(ui), &ui,
                      NULL);
      fprintf(stderr, "\t\tDevice Maximum Compute Units = %d\n", ui);

      clGetDeviceInfo(devices[d], CL_DEVICE_MAX_WORK_ITEM_DIMENSIONS,
                      sizeof(ui), &ui, NULL);
      fprintf(stderr, "\t\tDevice Maximum Work Item Dimensions = %d\n", ui);

      clGetDeviceInfo(devices[d], CL_DEVICE_MAX_WORK_ITEM_SIZES, sizeof(sizes),
                      sizes, NULL);
      fprintf(stderr, "\t\tDevice Maximum Work Item Sizes = %d x %d x %d\n",
              sizes[0], sizes[1], sizes[2]);

      clGetDeviceInfo(devices[d], CL_DEVICE_MAX_WORK_GROUP_SIZE, sizeof(size),
                      &size, NULL);
      fprintf(stderr, "\t\tDevice Maximum Work Group Size = %d\n", size);

      clGetDeviceInfo(devices[d], CL_DEVICE_MAX_CLOCK_FREQUENCY, sizeof(ui),
                      &ui, NULL);
      fprintf(stderr, "\t\tDevice Maximum Clock Frequency = %d MHz\n", ui);

      size_t extensionSize;
      clGetDeviceInfo(devices[d], CL_DEVICE_EXTENSIONS, 0, NULL,
                      &extensionSize);
      char *extensions = new char[extensionSize];
      clGetDeviceInfo(devices[d], CL_DEVICE_EXTENSIONS, extensionSize,
                      extensions, NULL);
      fprintf(stderr, "\nDevice #%d's Extensions:\n", d);
      for (int e = 0; e < (int)strlen(extensions); e++) {
        if (extensions[e] == ' ') extensions[e] = '\n';
      }
      fprintf(stderr, "%s\n", extensions);
      delete[] extensions;
    }
...
void SelectOpenclDevice() {
  ...
  for (int p = 0; p < (int)numPlatforms; p++) {
    ...
    for (int d = 0; d < (int)numDevices; d++) {
      ...
      clGetDeviceInfo(devices[d], CL_DEVICE_VENDOR_ID, sizeof(vendor), &vendor,
                      NULL);

      // select:

      if (bestPlatform <
          0)  // not yet holding anything -- we'll accept anything
      {
        bestPlatform = p;
        bestDevice = d;
      ...
      {
        if (bestDeviceType ==
            CL_DEVICE_TYPE_CPU)  // holding a cpu already -- switch to a gpu if
                                 // possible
        {
